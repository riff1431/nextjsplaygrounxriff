    ...
    <div className="w-full">
      ...
      <div className="grid grid-cols-4 gap-2">
        {packages.map((pkg) => (
          <div
            key={pkg.price}
            className={`bg-card/20 backdrop-blur-sm border rounded-lg p-2 md:p-3 text-center transition-all hover:scale-105 cursor-pointer ${
              pkg.popular
          ...
          >
            ...
            <div className="text-muted-foreground font-semibold text-[20px] uppercase tracking-wider mb-1.5">
              {pkg.votes} Votes
            </div>
            <button
              className={`w-full py-1 rounded-md font-display font-bold tracking-wider text-xs transition-all ${
                pkg.popular
