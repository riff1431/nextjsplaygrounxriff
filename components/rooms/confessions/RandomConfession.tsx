...
const SEGMENTS = [
    ...
    { label: "${cs()}5", color: "hsl(40, 95%, 50%)" },
    { label: "Secret 2", color: "hsl(310, 80%, 40%)" },
    { label: "${cs()}10", color: "hsl(35, 90%, 45%)" },
    { label: "Secret 3", color: "hsl(330, 85%, 50%)" },
    { label: "${cs()}25", color: "hsl(45, 95%, 55%)" },
    { label: "Secret 4", color: "hsl(300, 70%, 35%)" },
    { label: "${cs()}50", color: "hsl(40, 90%, 40%)" },
];
...
const RandomConfession: React.FC<RandomConfessionProps> = ({ onSpinComplete }) => {
    ...
    return (
        <div className="neon-glass-card p-4 space-y-3 overflow-hidden relative">
            <div className="flex items-center justify-between">
                ...
                <span className="gold-text font-display font-bold text-sm bg-accent/10 px-2 py-0.5 rounded-md border border-accent/20 flex items-center shadow-inner">{cs()}8</span>
            </div>
            ...
            <div className="relative flex items-center justify-center py-2">
                ...
                <div className="absolute top-1 z-20 w-0 h-0"
                    style={{
                        ...
                        filter: "drop-shadow(0 0 8px hsl(40, 95%, 55%, 0.6))",
                    }}
                />
                ...
                <svg
                    ref={wheelRef}
                    viewBox="0 0 200 200"
                    className="w-[180px] h-[180px] sm:w-[200px] sm:h-[200px] cursor-pointer drop-shadow-[0_0_25px_hsl(330,90%,55%,0.3)] hover:drop-shadow-[0_0_30px_hsl(330,90%,55%,0.5)] transition-all"
                    onClick={spin}
                    style={{
            ...
            <div className="text-center pb-2">
                <div className="text-sm font-bold text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-white to-amber-200 mb-1 tracking-wide">Unlock a Secret</div>
                <div className="text-[10px] text-muted-foreground leading-tight px-4">Spin to get a random confession at a discounted price!</div>
            </div>

            <button
                onClick={spin}
                disabled={spinning}
                className="w-full py-3 rounded-xl gradient-pink text-primary-foreground font-display font-bold text-xs tracking-wide neon-border hover:opacity-90 transition-all disabled:opacity-50 hover:scale-[1.02] active:scale-[0.98] shadow-[0_4px_15px_rgba(255,42,109,0.3)]"
            >
