...
const inNew = fs.readFileSync('app/rooms/truth-or-dare-creator/page.tsx', 'utf8'); // Using this just as reference if needed

const startHead = inOld.indexOf('export default function TruthOrDareCreatorRoom() {');
const endHead = inOld.indexOf('return (', startHead);
...
logic = logic.replace(
    /if \(!roomId && isLive\) {\s*return <div[^>]*>Loading Room...<\/div>;\s*}/, 
