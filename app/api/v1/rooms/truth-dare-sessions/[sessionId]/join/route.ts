...
export async function POST(
    ...
    try {
        // 1. Fetch session (admin client bypasses RLS)
        ...
        if (isPrivate) {
            ...
            if (existingReq) {
                ...
                if (existingReq.status === "approved") {
                    ...
                    return await addParticipantWithPayment(admin, session, user, sessionId);
                }
            }
            ...
            // Notify creator
            const { data: fanProfile } = await admin
                .from("profiles")
...
async function addParticipantWithPayment(
    ...
    if (price > 0 && creatorId) {
        const { data: result, error: payError } = await admin.rpc("transfer_funds", {
        ...
        // Record transaction
        await admin.from("truth_dare_session_transactions").insert({
            session_id: sessionId,
            user_id: user.id,
    ...
    await admin.from("truth_dare_unlocks").upsert({
