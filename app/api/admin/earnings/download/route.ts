...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        if (creatorId) {
            ...
            autoTable(doc, {
                ...
                body: [
                    ...
                    ['Creator Earned', `${SYM}${invoice.summary.creator_earned.toFixed(2)}`],
                    ['Platform Revenue', `${SYM}${invoice.summary.platform_earned.toFixed(2)}`],
            ...
            autoTable(doc, {
                ...
                body: invoice.lines.map((l: any) => [
                    ...
                    `${SYM}${l.gross_amount.toFixed(2)}`,
                    `${SYM}${l.creator_share.toFixed(2)}`,
                    `${SYM}${l.platform_share.toFixed(2)}`,
                    l.split_name || '-',
                ]),
                theme: 'grid',
        ...
        doc.text(`Total Platform Revenue: ${SYM}${Number(payoutsData.totals.total_platform_earned).toFixed(2)} | Total Creator Payouts: ${SYM}${Number(payoutsData.totals.total_creators_earned).toFixed(2)}`, 14, 33);
        ...
        autoTable(doc, {
            ...
            body: payoutsData.rows.map(r => [
                r.display_name,
                `@${r.username}`,
                `${SYM}${r.gross_collected.toFixed(2)}`,
                `${SYM}${r.creator_earned.toFixed(2)}`,
                `${SYM}${r.platform_earned.toFixed(2)}`,
                `${r.events_count}`,
